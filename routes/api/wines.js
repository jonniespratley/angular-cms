/**
 *
 * @author Jonnie Spratley, AppMatrix
 * @created 10/23/12
 */

/**
 * Resource - This is the resource object that contains all of the REST api methods for a full CRUD on a mongo db document.
 * 
 * REST METHODS:
 * 
 * HTTP		METHOD			URL								
 * GET 		findAll			http://localhost:3000/wines
 * GET 		findById		http://localhost:3000/wines/:id
 * POST 	add				http://localhost:3000/wines
 * PUT		update			http://localhost:3000/wines/:id
 * DELETE	destroy			http://localhost:3000/wines/:id
 */
var Resource = {
    host: '127.0.0.1',
	/**
	 * I enable logging or not.
	 */
	debug : true, 
	/**
	 * I am the interal logger.
	 */
	log : function(obj) {
		if(Resource.debug) {
			console.log(obj);
		}
	},
	/**
	 * I am the name of the database.
	 */
	databaseName : 'winedb',
	/**
	 * I am the name of this collection.
	 */
	name : 'wines',
	/**
	 * I am the example schema for this resources.
	 */
	schema : [{
		title : 'Wine 1', year : '2009', country : 'France', body : 'This is the body'
	}, {
		title : 'Wine 2', year : '2009', country : 'France', body : 'This is the body'
	}],
	/**
	 * I populate the document db with the schema.
	 */
	populateDb : function() {
		db.collection(Resource.name, function(err, collection) {
			collection.insert(Resource.schema, {
				safe : true
			}, function(err, result) {
				Resource.log(result);
			});
		});
	},
	/**
	 * I find all of the records
	 * @param {Object} req
	 * @param {Object} res
	 */
	findAll : function(req, res) {
		db.collection(Resource.name, function(err, collection) {
			collection.find().toArray(function(err, items) {
				Resource.log('findAll - ' + JSON.stringify(items));
				res.send(items);
			});
		});
	},
	/**
	 * I find one of the records by id.
	 * @param {Object} req
	 * @param {Object} res
	 */
	findById : function(req, res) {
		//get the id
		var id = req.params.id;
		//log
		Resource.log('Finding wine:' + id);
		//find
		db.collection(Resource.name, function(err, collection) {
			collection.findOne({
				'_id' : new BSON.ObjectID(id)
			}, function(err, item) {
				res.send(item);
			});
		});
	},
	/**
	 * I add a record to the collection
	 * @param {Object} req
	 * @param {Object} res
	 */
	add : function(req, res) {
		var data = req.body;
		//log
		Resource.log('Adding wine: ' + JSON.stringify(data));
		//Add the item to the collection
		db.collection(Resource.name, function(err, collection) {
			collection.insert(data, {
				safe : true
			}, function(err, result) {
				if(err) {
					res.send({
						'error' : 'An error has occurred'
					});
				} else {
					Resource.log('Success: ' + JSON.stringify(result[0]));
					res.send(result[0]);
				}
			});
		});
	},
	/**
	 * I update a record in the collection
	 * @param {Object} req
	 * @param {Object} res
	 */
	update : function(req, res) {
		var id = req.params.id;
		var data = req.body;
		//log
		Resource.log('Updating wine: ' + id + ' - ' + JSON.stringify(data));
		//Get the collection, find the record and update
		db.collection(Resource.name, function(err, collection) {
			collection.update({
				'_id' : new BSON.ObjectID(id)
			}, data, {
				safe : true
			}, function(err, result) {
				if(err) {
					res.send({
						'error' : 'An error has occurred'
					});
					console.log('Error updating ' + Resource.name + ': ' + err);
				} else {
					console.log('' + result + 'document(s) updated');
					//Send the updated item back
					res.send(data);
				}
			});
		});
	},
	/**
	 * I delete a record in the collection.
	 * @param {Object} req
	 * @param {Object} res
	 */
	destroy : function(req, res) {
		var id = req.params.id;
		Resource.log('Deleting ' + Resource.name + ' : ' + id);
		db.collection(Resource.name, function(err, collection) {
			collection.remove({
				'_id' : new BSON.ObjectID(id)
			}, {
				safe : true
			}, function(err, result) {
				if(err) {
					res.send({
						'error' : 'An error has occurred'
					});
					Resource.log('Error updating ' + Resource.name + ': ' + err);
				} else {
					res.send(req.body);
				}
			});
		});
	}
};
/**
 * Create the mongo server, database, and bson.
 */
var mongo = require('mongodb'); 
var Server = mongo.Server;
var Db = mongo.Db;
var BSON = mongo.BSONPure;

//new Db(new Server('localhost', 27017), {safe:false})

var server = new Server(Resource.host, 27017, {
	auto_reconnect : true,
	safe: false
});
var db = new Db(Resource.databaseName, server);
/**
 * Open the database and check for collection, if none
 * then create it with the schema.
 */
db.open(function(err, db) {
	if(!err) {
		//log
		console.log('Connected to ' + Resource.databaseName, db);
		db.collection(Resource.name, {
			safe : true
		}, function(err, collection) {
			if(err) {
				Resource.log('The collection doesnt exist. creating it with sample data...');
				Resource.populateDb();
			}
		});
	}
});
//Export to public api
exports.WinesResource = Resource;
