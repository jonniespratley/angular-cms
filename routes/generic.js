...
Resource = {
	 ...
	 */
	init: function () {
		server = new Server(Resource.host, Resource.port);
		database = new Db(Resource.databaseName, Resource.server, {
			auto_reconnect: true,
			safe: false
		});
	 ...
	 */
	populateDb: function () {
		database.collection(Resource.name, function (err, collection) {
	...
	findAll: function (col) {
		database.collection(col, function (err, collection) {
			collection.find().toArray(function (err, items) {
				...
				if (items) {
					Resource.log(col + ':findAll - ' + JSON.stringify(items));
					return items;
				}
			});
		});
